...
1. **Add the workflow.** Copy `assets/deploy-pages.yml` to
   ...
   - the `upload-pages-artifact` `path:` → a dedicated public site folder that
     contains `index.html` (for example `./site` or `./public`). Avoid deploying
     the whole course root unless it has been audited; course folders can contain
     source material, progress notes, learner profiles, and private examples.
...
- Prefer deploying a dedicated public folder, not the full generated course
  folder. If the full folder is deployed, review `SOURCE.md`, `user.json`,
  `progress.json`, `PROGRESS.md`, `materials/`, and any brownfield copy first.
